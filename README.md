...
Codex Auto Runner is a local recovery runner for long Codex goal sessions.

Its purpose is simple: when the Codex 5-hour quota window recovers, it automatically resumes the active goal task that was already in progress, keeps the original Codex thread and context intact, and continues pushing the work forward until the weekly quota is exhausted, the task is complete, or human judgment is required.

It does not bypass quotas, modify account limits, or replace Codex. Instead, it makes recovered quota windows useful. If a long migration, refactor, product build, or debugging session stops because the 5-hour window is exhausted, Codex Auto Runner waits locally, verifies the real quota state when the window resets, restores the current goal to active when needed, and sends the smallest possible continuation instruction so Codex can read the existing context and keep going.

What it gives you:

- Automatic 5-hour quota recovery detection.
- Same-thread resume for active Codex goal sessions.
- Weekly quota drive mode for long-running work.
- Goal-mode detection, so sessions without a Codex goal cannot be scheduled by mistake.
- Local-only daemon, dashboard, SQLite state, and token-protected API.
- Privacy-safe runtime discovery of Codex, with no author machine paths or personal credentials stored in the repository.

Codex Auto Runner is an unofficial local companion for Codex. It is built for people who want their Codex goals to continue the moment quota is available again, without wasting the next recovery window waiting for someone to press continue.
